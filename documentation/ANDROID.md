...
9. Native alarm records are independent of the WebView and are restored after restart, app replacement, manual time change, time-zone change, and user unlock. Old Capacitor-plugin alarms are cancelled during migration.
10. No exact-alarm permission is requested. The native batch uses `AlarmManager.setAndAllowWhileIdle`; Android may deliver it shortly after 9:00 AM when battery optimization or Doze mode requires it. Force-stopping the app in Android settings prevents Android alarms until the app is opened again.
11. CardNest verifies that the notification channel is enabled and that Android persisted the complete schedule. Any failure is shown on the Reminders and Settings pages instead of displaying a false “Notifications ready” state.
