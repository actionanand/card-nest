...
PDF and CSV export do not add another npm dependency. Browser PDF export uses the print dialog, browser CSV export uses the standard Blob download API, and Android uses the custom Capacitor bridge generated by `scripts/patch-android-export.mjs`.
...
- Transaction exports support the current budget cycle, three months, six months, one year, or all stored history. PDF statements preserve the active transaction filters and include monthly totals; UTF-8 CSV files include spreadsheet-safe headings and escaped values. Statistics PDF exports include income, expense, remaining money, savings rate, category/source breakdowns, and largest expenses.
